...
pub fn compare_strings(str1: &CStr, str2: &CStr) -> i32 {
    unsafe { pebble_sys::strcmp(str1.as_ptr(), str2.as_ptr()) }
}

pub fn compare_strings_bytes(str1: &CStr, str2: &CStr, max_bytes: usize) -> i32 {
    unsafe { pebble_sys::strncmp(str1.as_ptr(), str2.as_ptr(), max_bytes as u32) }
}
...
pub unsafe fn copy_strings<'a>(source: &CStr, dest: *mut c_char) -> &'a str {
    unsafe {
        let ptr = pebble_sys::strcpy(dest, source.as_ptr());
        ptr_to_str(ptr)
    }
}
...
pub unsafe fn copy_strings_bytes<'a>(
    ...
    unsafe {
        let ptr = pebble_sys::strncpy(dest, source.as_ptr(), max_bytes as u32);
        ptr_to_str(ptr)
    }
}
...
pub unsafe fn concat_strings<'a>(source: &CStr, dest: *mut c_char) -> &'a str {
    unsafe {
        let ptr = pebble_sys::strcat(dest, source.as_ptr());
        ptr_to_str(ptr)
    }
}
...
pub unsafe fn concat_strings_bytes<'a>(
    ...
    unsafe {
        let ptr = pebble_sys::strncat(dest, source.as_ptr(), max_bytes as u32);
        ptr_to_str(ptr)
    }
}

pub fn string_length(string: &CStr) -> usize {
    unsafe { pebble_sys::strlen(string.as_ptr()) as usize }
}
